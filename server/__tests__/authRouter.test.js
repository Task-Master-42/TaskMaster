...
describe('Auth Routes', () => {
  ...
  describe('GET /auth/check', () => {
    it('should return isLoggedIn as true if there\'s a valid cookie', async () => {
      ...
      await agent.post('/api/auth/login').send({username, password: 'egg'});
    ...
    it('should return isLoggedIn as false if cookie informatin is incorrect', async () => {
      const agent = request.agent(app);
      await agent.post('/api/auth/login').send({username, password: 'egg'});
  ...
  describe('GET /auth/logout', () => {
    it('should successfully end a session on logout request', async () => {
      ...
      const checkCookie = await agent.get('/api/auth/check');
      expect(checkCookie.status).toBe(200);
      expect(checkCookie.body).toHaveProperty('isLoggedIn', false);
    });
  })
})
